...
def release_day(releaseInfo):
    if '上线首日' == releaseInfo:
        return 0
    rex = r'\d+'
    tmp = re.search(rex, releaseInfo)
...
def zongyi_title(name:str):
    ...
    if ' ' in name:
        return name.split()[0], '0', rep[name.split()[-1]]
    else:
        return name, str(time.localtime().tm_year), 1
...
def guochanju():
    try:
        ...
        URL = "https://piaofang.maoyan.com/dashboard/webHeatData?seriesType=&platformType=&showDate=2&dateType=0&rankType=0&User-Agent=TW96aWxsYS81LjAgKFdpbmRvd3MgTlQgMTAuMDsgV2luNjQ7IHg2NCkgQXBwbGVXZWJLaXQvNTM3LjM2IChLSFRNTCwgbGlrZSBHZWNrbykgQ2hyb21lLzE0NC4wLjAuMCBTYWZhcmkvNTM3LjM2&index=926&channelId=40009&sVersion=2&signKey=fcda3d874bde37c6173c1edadcc22bfb&WuKongReady=h5"
        # 设置 User-Agent 模拟浏览器
        HEADERS = {
        ...
        have_new_count = 0
        num = 1
        for i in response['dataList']['list']:
            name = i['seriesInfo']['name']
            occurDays = release_day(i['seriesInfo']['releaseInfo'])
            year = time.localtime().tm_year
            title = f'{name} ({year})'
            if num < 6 and occurDays < 4:
                if is_chinese(name[-2]) and is_num(name[-1]):
                    season = int(name[-1])
...
def zongyi():
    try:
        # 签到页面的 URL（通常是点击签到按钮时请求的那个地址）
        URL = "https://piaofang.maoyan.com/dashboard/webHeatData?seriesType=2&platformType=&showDate=2&dateType=0&rankType=0&User-Agent=TW96aWxsYS81LjAgKFdpbmRvd3MgTlQgMTAuMDsgV2luNjQ7IHg2NCkgQXBwbGVXZWJLaXQvNTM3LjM2IChLSFRNTCwgbGlrZSBHZWNrbykgQ2hyb21lLzE0NC4wLjAuMCBTYWZhcmkvNTM3LjM2&index=598&channelId=40009&sVersion=2&signKey=495101754d420ff072892d87a0ef915e&WuKongReady=h5"
        # 设置 User-Agent 模拟浏览器
        HEADERS = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/143.0.0.0 Safari/537.36",
        }
        ...
        num = 1
        for i in response['dataList']['list']:
            name, year, season = zongyi_title(i['seriesInfo']['name'])
            occurDays = release_day(i['seriesInfo']['releaseInfo'])
            title = f"{i['seriesInfo']['name']} ({time.localtime().tm_year})"
            if num < 5 and occurDays < 5:
                is_new = check_and_save(title)
                if is_new:
                    payload = {
                        "name": name,
                    ...
                    if mp.add_sub(payload):
                        pushplus(pushplus_token, '最新综艺通知', title)
                        have_new_count += 1
                    else:
                        delete_data(title)
            num += 1
...
if __name__ == "__main__":
    ...
    DB_FILE = os.path.join(BASE_DIR, "maoyan.db")
